        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-pink-50 dark:bg-pink-200 overflow-hidden shadow-md rounded-xl border border-pink-300">
                <div class="p-6 text-gray-900 dark:text-gray-800">
                    ...
                    <form action="{{ route('surat_keluar.update', $suratKeluar->id_surat_keluar) }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')

                        <div class="mb-4">
                            <label for="no_surat_keluar" class="block text-sm font-medium text-pink-800">Nomor Surat</label>
                            <input type="text" name="no_surat_keluar" id="no_surat_keluar"
                                value="{{ old('no_surat_keluar', $suratKeluar->no_surat_keluar) }}"
                        ...
                        <div class="mb-4">
                            ...
                            <select name="jenis_surat_id" id="jenis_surat_id"
                                ...
                                <option value="{{ $jenis->id }}"
                                    {{ old('jenis_surat_id', $suratKeluar->jenis_surat_id) == $jenis->id ? 'selected' : '' }}>
                                    {{ $jenis->keterangan }}
                                </option>
                                @endforeach
                            </select>
                        ...
                        <div class="mb-4">
                            <label for="file" class="block text-sm font-medium text-pink-800">File Baru (PDF, Doc)</label>
                            <p class="text-sm mb-2 text-pink-700">File saat ini:
                                <a href="{{ route('surat_keluar.download', $suratKeluar->id_surat_keluar) }}"
                                    class="text-indigo-500 underline hover:text-indigo-700">Unduh File Saat Ini</a>
                            </p>
                            <input type="file" name="file" id="file"
                                class="block w-full text-sm border border-pink-300 bg-white rounded-xl shadow-sm focus:outline-none focus:border-pink-500 focus:ring-pink-500">
                        </div>
